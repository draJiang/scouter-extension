...
import General from "../Options/section/General"
...
function Main() {
  ...
  useEffect(() => {
    ...
    (async () => {
      const uInfo: userInfoType = await getUserInfo()
      setUserInfo(uInfo)
  ...
  return (
    <div className='w-96'>
      <header className=' flex items-center flex-row p-3 border-b border-b-gray-200 bg-white sticky top-0 z-50'>
        <div className=' flex-1'>
          <img className=' w-5 h-5' src='icon128.png' />
        </div>
        <div>
          <button
            className='flex flex-row items-center border-4 border-transparent box-border'
      ...
      <div className='p-5 mt-1'>
        <UserInfoContext.Provider value={{ user: userInfo, anki: null }}>
          <General settings={settings} saveOptions={saveOptions} />
